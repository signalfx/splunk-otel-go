...
func TestDefaultSpanStartOptions(t *testing.T) {
	...
	assert.Equal(t, ssc.SpanKind(), trace.SpanKindClient)
	assert.Contains(t, ssc.Attributes(), semconv.DBSystemKey.String("leveldb"))
	assert.Contains(t, ssc.Attributes(), semconv.NetTransportInProc)
}
