...
func WithTraceResponseHeader(v bool) otelhttp.Option {
	return optionWrapper{apply: func(cfg *config) {
		cfg.TraceResponseHeaderEnabled = v
	}}
}

// config represents the available Splunk specific configuration options.
type config struct {
	TraceResponseHeaderEnabled bool
}

// optionWrapper provides a convenience wrapper for Splunk specific options.
type optionWrapper struct {
	otelhttp.Option
	apply func(*config)
}

// newConfig creates a new config struct and applies Splunk specific options to it.
func newConfig(opts ...otelhttp.Option) *config {
	...
	for _, opt := range opts {
		// apply only Splunk specific options
		if splunkOpt, ok := opt.(optionWrapper); ok {
			splunkOpt.apply(c)
		}
	}
	return c
